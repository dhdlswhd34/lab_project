...
int on_xml_parse(void* a,void* b,void* c,void* d)
{
  ...
  if ( strncmp((char*)c, "CONFIG", 6) == 0 )
  {
    ...
    else if ( *enable=='1' && strncmp(out, "HTTPD", strlen(out))==0 )
    {
      info->arg[XHTTPD][XM_ENABLE][0] = 'E';
      mtifXMLGetAttr(&p->m, 0, "sdelay",    info->arg[XHTTPD][XM_DELAY_0]);
      mtifXMLGetAttr(&p->m, 0, "ldelay",    info->arg[XHTTPD][XM_DELAY_1]);
      mtifXMLGetAttr(&p->m, 0, "ip",        info->arg[XHTTPD][XM_IP]);
      mtifXMLGetAttr(&p->m, 0, "port",      info->arg[XHTTPD][XM_PORT]);
      mtifXMLGetAttr(&p->m, 0, "home",      info->arg[XHTTPD][XM_HOME]);
      mtifXMLGetAttr(&p->m, 0, "index",     info->arg[XHTTPD][XM_INDEX]);
      xmsgq_set(p, (void*)MAKELONG(INIT, XHTTPD), (void*)info, 0);
    }
    ...
    else if ( *enable=='1' && strncmp(out, "SHM", strlen(out))==0 )
    {
      info->arg[XSHM][XM_ENABLE][0] = 'E';
      mtifXMLGetAttr(&p->m, 0, "delay",     info->arg[XSHM][XM_DELAY_0]);
      mtifXMLGetAttr(&p->m, 0, "key",       info->arg[XSHM][XM_KEY]);
      mtifXMLGetAttr(&p->m, 0, "size",      info->arg[XSHM][XM_SIZE]);
      mtifXMLGetAttr(&p->m, 0, "count",     info->arg[XSHM][XM_COUNT]);
      xmsgq_set(p, (void*)MAKELONG(INIT, XSHM), (void*)info, 0);
    }
    else if ( *enable=='1' && strncmp(out, "SHM_S", strlen(out))==0 )
    {
      info->arg[XSHM_S][XM_ENABLE][0] = 'E';
      mtifXMLGetAttr(&p->m, 0, "delay",     info->arg[XSHM_S][XM_DELAY_0]);
      mtifXMLGetAttr(&p->m, 0, "key",       info->arg[XSHM_S][XM_KEY]);
      mtifXMLGetAttr(&p->m, 0, "size",      info->arg[XSHM_S][XM_SIZE]);
      mtifXMLGetAttr(&p->m, 0, "count",     info->arg[XSHM_S][XM_COUNT]);
      xmsgq_set(p, (void*)MAKELONG(INIT, XSHM_S), (void*)info, 0);
    }
